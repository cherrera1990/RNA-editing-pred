#PART 3 of 6: please, make sure every process ends before running the next part of this script. When using the small dataset example, take into account that steps 4 to 6 will take
...
#this script generates balanced datasets for the Random Forest by choosing randomly from the positive and negative dataset files the same amount of positive and negative cases. It also creates a random selection of
#sequences for smaller Neural Network datasets.
#bote: For the purpose of the small example, the larger Random Forest datasets have been commented out inside the scripts. If you wish to get larger Random Forest datasets, you may uncomment and modify the bash-random-lines.sh script.
bash bash-random-lines.sh $4
